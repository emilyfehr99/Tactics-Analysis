#!/usr/bin/env python3
"""
Clean Fast Video to Tactics: Quick Hockey Analysis

This tool processes EVERY frame quickly by calling Computer-Vision-for-Hockey
with optimal parameters, without modifying the original project.
"""
...
def run_clean_fast_tracking(video_path: str, cv_hockey_path: str = "../Computer-Vision-for-Hockey", max_frames: int = 200) -> str:
    """Run fast tracking using Computer-Vision-for-Hockey with optimal settings."""
    print(f"⚡ CLEAN FAST TRACKING: {Path(video_path).name}")
    ...
    try:
        # Change to CV-Hockey directory and run tracking directly
        ...
        print(f"⚡ Starting clean fast tracking...")
        print(f"   Target: Under 1 minute for {max_frames} frames...")
        
        start_time = time.time()
        
        # Call process_clip.py directly with optimal parameters
        cmd = [
        ...
        try:
            ...
            print("⏰ Tracking taking longer than expected, but continuing...")
        ...
        print(f"⚡ Clean fast tracking completed in {processing_time:.1f} seconds!")
        ...
        print(f"❌ Clean fast tracking failed: {e}")
...
def run_quick_tactical_analysis(tracking_data_path: str):
    """Run quick tactical analysis."""
    print(f"🔍 Quick tactical analysis...")
...
def display_clean_results(results: dict, tracking_file: str):
    """Display clean analysis results."""
    print("\n📊 CLEAN TACTICAL ANALYSIS RESULTS")
    print("=" * 50)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clean Fast Video to Tactics: Quick Hockey Analysis"
    )
    parser.add_argument(
        "video_path",
    ...
    print("⚡ CLEAN FAST VIDEO TO TACTICS: QUICK HOCKEY ANALYSIS")
    print("=" * 70)
    ...
    print(f"⚡ Target Speed: Under 1 minute for complete analysis")
    print(f"🔒 No modifications to Computer-Vision-for-Hockey")
    print()
    
    try:
        start_time = time.time()
        
        # Step 1: Clean fast tracking (every frame, no modifications)
        tracking_file = run_clean_fast_tracking(
            str(video_path), 
            args.cv_hockey_path,
            args.max_frames
        )
        ...
        results = run_quick_tactical_analysis(tracking_file)
        
        # Step 3: Display results
        display_clean_results(results, tracking_file)
        ...
        if total_time < 60:
            print("🎯 TARGET ACHIEVED: Under 1 minute!")
        else:
            print(f"⏰ Target missed by {total_time - 60:.1f} seconds")
        
    except Exception as e:
        print(f"❌ Clean fast analysis failed: {e}")
        sys.exit(1)
